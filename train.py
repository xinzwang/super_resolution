...
def train(args):
	...
	scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=20, threshold=1e-4, min_lr=1e-5)
	...
	for epoch in range(args.epoch):
		...
		for i, (lr, hr)in enumerate(tqdm(dataloader)):
			...
			if i % 20 == 0:
				logger.info('epoch:%d batch:%d loss:%.5f'%(epoch,i,loss.item()))
		mean_loss = np.mean(total_loss)
		scheduler.step(mean_loss)
		# test
		psnr, ssim, mse = test(model, test_dataloader, device)
		# log
		logger.info('epoch:%d mean_loss:%.5f psnr=%.5f ssim=%.5f mse=%.5f' % (epoch, mean_loss, psnr, ssim, mse))
		# save ckpt
		save_path = ckpt_path + 'epoch=%d_psnr=%.5f_ssim=%.5f_mse=%.5f.pt' % (psnr, ssim, mse)
		torch.save({
	...
	psnr, ssim, mse = test(model, test_dataloader, device)
	logger.info('epoch:%d mean_loss:%.5f psnr=%.5f ssim=%.5f mse=%.5f' % (epoch, mean_loss, psnr, ssim, mse))
	save_path = ckpt_path + 'final_epoch=%d_psnr=%.5f_ssim=%.5f_mse=%.5f.pt' % (psnr, ssim, mse)
	torch.save({
		'modelname': args.model,
		'model': model,
		'dataset': args.dataset
