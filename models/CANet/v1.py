...
class CANet(nn.Module):
	def __init__(self, channels=3, scale_factor=2, sft_mode='affine'):
		...
		sft = SFTAffineLayer if sft_mode == 'affine' else SFTConcatLayer
		...
		# head conv
		self.conv_head = default_conv(in_channels=channels, out_channels=n_feats, kernel_size=3)
		...
		# tail conv
		self.conv_tail = default_conv(in_channels=n_feats, out_channels=channels, kernel_size=3)
		return

	def forward(self, x):
		lr_knn = self.conv_head(x)
